...
void setup() {
    ...
    for(int n=0;n<NUM_SAMPLES;n++) {    
        xs[n] = 
        (double)random(RANDMAX)/RANDMAX*(MAX_X - MIN_X)+MIN_X;
    ...
    dnn_learning(x_train, y_train, NUM_LEARNING, 600);

    double * y_pred = (double *)malloc(NUM_THINKING*sizeof(double));

    dnn_think(x_test, y_pred, NUM_THINKING);

    lcdClear(TFT_BLACK);
    lcdPlot(x_test, y_test, TFT_GREEN, NUM_THINKING);
    lcdPlot(x_test, y_pred, TFT_RED, NUM_THINKING);
